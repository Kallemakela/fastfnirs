...
def print_results(y, preds, event2name=None):
	yc = np.concatenate([yi for yi in y.values()])
	print(classification_report(yc, preds, target_names=[event2name[c] for c in np.unique(yc)]))
	print(confusion_matrix(yc, preds))
	print()
	for condition in np.unique(yc):
		print(f'{event2name[condition]:5}: {np.mean(preds[yc == condition] == condition):.3f}')
