...
def interpolate_bads_nirs(inst, method='nearest', exclude=(), verbose=None):
	...
	picks_nirs = sorted(picks_nirs) # in new versions of mne, _check_channels_ordered returns channels in different order than data
	if len(picks_nirs) == 0:
		return
	...
	if method == 'nearest':
		...
		for bad in picks_bad:
			...
			all_closest_idxs = np.argwhere(np.isclose(dists_to_bad, np.min(dists_to_bad)))
...
def process_raw_od(
	...
	with np.errstate(invalid='ignore'): # some channels have all zeros, they will be eliminated
		sci = mne.preprocessing.nirs.scalp_coupling_index(raw)
	raw.info['bads'] = list(compress(raw.ch_names, (np.isnan(sci) | (sci <= sci_threshold))))
	if verbose:
...
def create_epochs_from_raw(
	...
	**kwargs,
	...
	epochs.metadata['bad_epoch_reason'] = epochs.copy().drop_bad(reject=reject_criteria, verbose=verbose).drop_log
	epochs.metadata['is_bad_epoch'] = epochs.metadata['bad_epoch_reason'].astype(str) != '()'
	epochs.metadata['bad_channels'] = list(np.tile(epochs.info['bads'], (len(epochs.metadata), 1))) 
