...
def handle_duplicate_events(events):
    ...
    for i, (sample, _, _) in enumerate(events):
        if i > 0 and sample == events[i - 1, 0]:
            events[i, 0] += 1
            logger.info(f"Found duplicate event at sample {sample}, moved latter event to sample {events[i, 0]}")
    return events

def get_subjects(root_path):
    return pd.read_csv(Path(root_path) / "participants.tsv", sep="\t")['participant_id'].values
...
def get_ch_type(root_path):
    ...
    bids_ch_types = pd.read_csv(Path(root_path) / subject / 'nirs' / f'{subject}_task-{task}_channels.tsv', sep='\t')['type'].values
        ...
        for task in tasks:
            ...
            for ch_type in bids_ch_types:
                if ch_type != excepted_ch_type:
                    raise ValueError(f'found ch_type {ch_type} in {subject} {task} but expected {excepted_ch_type}')
